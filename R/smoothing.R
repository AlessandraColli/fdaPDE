...
#' @param locations A #observations-by-2 matrix in the 2D case and #observations-by-3 matrix in the 2.5D case, where 
#' each row specifies the spatial coordinates \code{x} and \code{y} (and \code{z} in 2.5D) of the corresponding 
...
#' a vector with length #points.}
#' }
#' For 2.5D, only the Laplacian is available (\code{PDE_parameters=NULL}). 
#' @param incidence_matrix A #regions-by-#triangles matrix where the element (i,j) equals 1 if the j-th 
#' triangle is in the i-th region and 0 otherwise.
...
#' data = fs.test(mesh$nodes[,1], mesh$nodes[,2], exclude = FALSE) +
